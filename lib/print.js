...
export default async function (m, conn = { user: {} }) {
    ...
    let chatName = chat ? (m.isGroup ? 'Grupo: ' + chat : 'Chat individual: ' + chat) : '';
...
if (/document/i.test(m.mtype)) console.log(`➜ ${m.msg.fileName || m.msg.displayName || 'DOC'}`)
else if (/ContactsArray/i.test(m.mtype)) console.log(`→ ${' ' || ''}`)
else if (/contact/i.test(m.mtype)) console.log(`▸ ${m.msg.displayName || ''}`)
else if (/audio/i.test(m.mtype)) {
const duration = m.msg.seconds
console.log(`➜ㅤ(PTT ' : '→ㅤ('}Audio mp3) ${Math.floor(duration / 60).toString().padStart(2, 0)}:${(duration % 60).toString().padStart(2, 0)}`)
}
console.log()
}
