...
export default async function (m, conn = { user: {} }) {
    ...
    let chatName = chat ? (m.isGroup ? 'Partido de Equipo: ' + chat : 'Entrenamiento Individual: ' + chat) : '';

    /* # Ignorar mensajes del bot */
    if (m.sender === conn.user?.jid) return;
    console.log(`${chalk.black(chalk.bgBlueBright('%s'))} ${chalk.blueBright(oraColombia)}

➜ㅤ- ${chalk.blueBright('%s')}
→ - ${chalk.blueBright('%s [%s %sB]')}
▸ - ${chalk.white('%s')}
...
if (/document/i.test(m.mtype)) console.log(`➜ ${m.msg.fileName || m.msg.displayName || 'Documento de Estrategia'}`)
else if (/ContactsArray/i.test(m.mtype)) console.log(`→ ${' ' || ''}`)
else if (/contact/i.test(m.mtype)) console.log(`▸ ${m.msg.displayName || ''}`)
else if (/audio/i.test(m.mtype)) {
const duration = m.msg.seconds
console.log(`➜ㅤ(PTT ' : '→ㅤ('}AUDIO DE ENTRENAMIENTO) ${Math.floor(duration / 60).toString().padStart(2, 0)}:${(duration % 60).toString().padStart(2, 0)}`)
}
console.log()
}
